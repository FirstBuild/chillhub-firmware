      ...
      if (hardware_version) {
        if (software_version) {
          ...
          var gotRoot = function(rootSnap) {
             ...
             if (rootSnap.val() != null && rootSnap.val().created && rootSnap.val().updated) {
         ...
         if (type) {
            ...
            attachment.once('value', 
                  function(snap) {
                     ...
                     if (snap.val() != null && snap.val().created) {
                     ...
                     attachment.onDisconnect().update({"status": "disconnected"});
                     callback(null, {
...
var createAttachmentResource = function(attachment, key, initialValue, onChange, callback) {
   if (callback === undefined) {
      ...
      attachment.update(data, function(err) {
         if (err) {
            console.log("Resource creation failed for resource " + key);
